...
/*
 * Fast rank from (n,k)-subsets to contiguous integers.
 */
struct subset_index
{
    size_t n, k;
...
struct Solver
{
    ...
    vector <size_t> context_free_group; // previous member in group, or -1 if none
    ...
    string trim_context(string s)
    {
        if (s.size() > context_length) {
            return s.substr(s.size() - context_length);
        } else {
            return s;
        }
    }
    ...
    void generate_context_update_()
    {
        for (size_t i = 0; i < items.size(); ++i) {
            ...
            for (size_t c = 0; c < contexts.size(); ++c) {
                ...
                upd.push_back(make_pair(context_index[trim_context(new_context)], added_length));
            }
    ...
    void init_context()
    {
        ...
        context_free_group.clear();

        context_length = 1;
        vector <size_t> max_overlap(items.size());
        for (size_t i = 0; i < items.size(); ++i) {
            const string& item = items[i];
            for (const string& item2: items) {
                if (item == item2) continue;
                size_t overlap = item.size() + item2.size() - join_strings(item2, item).size();
                max_overlap[i] = max(max_overlap[i], overlap);
            }
            // also check initial context
            size_t overlap = item.size() + context0.size() - join_strings(context0, item).size();
            max_overlap[i] = max(max_overlap[i], overlap);

            // global max
            context_length = max(context_length, max_overlap[i]);
        }
        DEBUG("context_length = %zu\n", context_length);

        context_free_group.resize(items.size(), size_t(-1));
        for (size_t i = 0; i < items.size(); ++i) {
            if (max_overlap[i] == 0) {
                for (size_t j = i-1; j+1 > 0; --j) {
                    if (max_overlap[j] == 0
                        && items[i].size() == items[j].size()
                        && (trim_context(items[i]) == trim_context(items[j])
                            || items[i].substr(1) == items[j].substr(1)))
                    {
                        DEBUG("  NP group: %s =NP= %s\n", items[i].c_str(), items[j].c_str());
                        context_free_group[i] = j;
                        break;
                    }
                }
            }
        ...
        context0 = trim_context(context0);
        generate_contexts_();
    ...
    size_t solve()
    {
        ...
        // Main tables. Nearly all memory usage goes into these
        ...
        for (size_t k = 0; k < items.size(); ++k) {
            ...
            // Sanity check that subset_index ranks subsets in
            // lexicographic order
            if (false) {
                str_set_t s = (str_set_t(1) << (k+1)) - 1;
                for (size_t i = 0; i < comb_k1; ++i) {
                    assert (k1_index.rank(s) == i);
            ...
            if (k == 0) {
                ...
                for (size_t i = 0; i < items.size(); ++i) {
                    ...
                    context_t c1;
                    size_t length;
                    // only add first item in each context-free group
                    if (context_free_group[i] != size_t(-1)) {
                        continue;
                    }
                    ...
                    for (size_t length: k_solns[c]) {
                        ...
                        if (length) {
                            for (size_t i = 0; i < items.size(); ++i) {
                                ...
                                // Only add next item in each context-free group
                                // if earlier items are present
                                if (context_free_group[i] != size_t(-1) &&
                                    !str_set_member(s, context_free_group[i])) {
                                    continue;
                                }
                                // New item i
